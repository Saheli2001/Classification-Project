...
stats <- function(C, model){      # C := Confusion Matrix and Statistics
  t <- C$table
...
confusionMatrix(ifelse(phat_test >= p1_opt, 1, 0) %>% 
                  as.factor(), test_data$Diabetes_012) -> C_t

glue("Accuracy in test data: {a}",
     a = C_t$overall[[1]] %>% percent())
...
### ROC curves:
ROC_func <- function(m,name){
  plot(1 - m$Specificity, m$Sensitivity, type = 'l',
       main = paste('ROC curve ||',name,'data'), 
...
# fitting of model:
dtree <- rpart(Diabetes_012 ~ ., data = train_data,method='class')
summary(dtree)
...
imp_vars <- c('HighBP','GenHlth','HighChol','BMI','Age','Income')
...
stats(C_t, 'Logistic Regression')  # Line 200
stats(C, 'Decision Tree')
##############################################################
...
rf_initial <- randomForest(Diabetes_012 ~ ., data = train_data,
                           ntree = 300, mtry=4) 
importance(rf_initial) 
varImpPlot(rf_initial) #BMI is most important
# Plot of errors:
...
# Model metrics:
confusionMatrix(test_data$Diabetes_012, diab_pred) -> C
stats(C, 'Random Forest')  # Line 200
